...
const SITE_PAGE_YIELDS = [
  ...
  },
  {
    site: "publi24.ro",
    itemsPerPage: 20,
    note: "Search pages currently expose 20 ItemList results by default, with a larger page-size option in the UI.",
    recommended: {
      method: "Direct HTML fetch + JSON-LD/local parsing",
      firecrawlCreditsPerPage: 0,
      why: "Current search pages expose ItemList JSON-LD and total-result metadata directly, so Firecrawl credits are unnecessary for normal search pages.",
      avoid: "Avoid paid scrape unless Publi24 removes the server-rendered ItemList payload."
    }
  }
];
...
export function buildBudgetPayload(multiplier = DEFAULT_MULTIPLIER) {
  ...
  return {
    ...
    assumptions: [
      ...
      "Site yields reflect live first-page observations for the query 'bicicleta' on April 2, 2026.",
