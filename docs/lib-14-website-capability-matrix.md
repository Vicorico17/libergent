...
| Pricing/plans | Free vs PRO 29 RON tiers with limits/features (`ui/src/components/sections/Pricing.tsx`, `ui/src/components/sections/FAQ.tsx`) | No billing provider, no subscription model, no entitlement checks in app/API (`src/server.js`, `src/worker.js`, `ui/src/app/auth/page.tsx`) | planned | CEO + CTO | Current pricing is narrative-only; claims should not imply active monetization gates. |
| Realtime alerts | "Alerte în timp real" / immediate notifications (`ui/src/components/sections/FeaturesGrid.tsx`, `ui/src/components/sections/FAQ.tsx`) | No alert scheduler, notification service, or user alert preferences in codebase | planned | CTO | Keep as roadmap item until delivery path exists (email/push + trigger pipeline). |
...
2. CTO monetization/auth spike to convert pricing claims into enforceable product behavior.
3. CTO integration plan for alerts pipeline before real-time messaging is shown as current capability.
